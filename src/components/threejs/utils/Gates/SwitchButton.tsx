...
import { FlipFlopCanvasStore, SwitchKey, addLine, removeLine, selectDot, toggleSwitch } from "../../../../store/slices/FlipFlopCanvasSlice";
...
const SwitchButton = ({ meshPosition, id }: SwitchProps) => {
    ...
    const handleSwitchButton = (id : number)=>{
        if(id == 5 || id == 6)return;
        const switchId : SwitchKey = `SWITCH${id}` as SwitchKey; 
    ...
    const getSwitchColor = (id : number)=>{
        return flipFlopState.switch[`SWITCH${id}` as SwitchKey]?'#8b0000':'red';
    }
